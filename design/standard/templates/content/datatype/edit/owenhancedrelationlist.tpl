...
{if $class_content.selection_type|ne( 0 )} 
        ...
        {if ne( count( $nodesList ), 0)}
            <select name="{$attribute_base}_data_object_relation_list_{$attribute.id}[]">
                {if $attribute.contentclass_attribute.is_required|not}
                    <option value="no_relation" {if eq( $attribute.content.relation_list|count, 0 )} selected="selected"{/if}>
    ...
    {if $attribute.contentclass_attribute.is_required|not}
        <input type="radio" name="{$attribute_base}_data_object_relation_list_{$attribute.id}[]" value="no_relation"
               {if eq( $attribute.content.relation_list|count, 0 )} checked="checked"{/if}>{'No relation'|i18n( 'design/standard/content/datatype' )}<br />
    {/if}
    {foreach $nodesList as $node}
    ...
    {if and( is_set( $class_content.default_placement.node_id ), ne( 0, $class_content.default_placement.node_id ), ne( '', $class_content.object_class ) )}
        ...
        {if and( is_set( $defaultNode ), $defaultNode.can_create )}
            ...
            <input class="button" type="button" value="Create New" name="CustomActionButton[{$attribute.id}_new_object]"
                   onclick="var divfield = document.getElementById('create_new_object_{$attribute.id}');
                           divfield.style.display = 'block';
                           var editfield = document.getElementById('attribute_{$attribute.id}_new_object_name');
                           editfield.focus();
...
{else}    {* Standard mode is browsing *}
    ...
    <div class="block" id="ezobjectrelationlist_browse_{$attribute.id}">
        ...
        {if eq( ezini( 'BackwardCompatibilitySettings', 'AdvancedObjectRelationList' ), 'enabled' )}
            {if $attribute.content.relation_list}
                <table class="list" cellspacing="0">
                    <tr class="bglight">
                        <th class="tight"><img src={'toggle-button-16x16.gif'|ezimage} alt="{'Invert selection.'|i18n( 'design/standard/content/datatype' )}" onclick="ezjs_toggleCheckboxes(document.editform, '{$attribute_base}_selection[{$attribute.id}][]');
                                return false;" title="{'Invert selection.'|i18n( 'design/standard/content/datatype' )}" /></th>
                    ...
                    {foreach $attribute.content.relation_list as $relation_index => $relation sequence array( bglight, bgdark ) as $sequence}
                        <tr class="{$sequence}">
                            {if $relation.is_modified}
                                {* Remove. *}
                                <td>
                                    <input id="ezcoa-{if ne( $attribute_base, 'ContentObjectAttribute' )}{$attribute_base}-{/if}{$attribute.contentclassattribute_id}_{$attribute.contentclass_attribute_identifier}_remove_{$relation_index}" class="ezcc-{$attribute.object.content_class.identifier} ezcca-{$attribute.object.content_class.identifier}_{$attribute.contentclass_attribute_identifier}" type="checkbox" name="{$attribute_base}_selection[{$attribute.id}][]" value="{$relation.contentobject_id}" />
                                ...
                                <td colspan="4">
                                    ...
                                    <fieldset>
                                        ...
                                        {foreach $version.contentobject_attributes as $version_attribute}
                                            <div class="block">
                                                {if $version_attribute.display_info.edit.grouped_input}
                                                    <fieldset>
                                                        <legend>{$version_attribute.contentclass_attribute.name}</legend>
                                                        {attribute_edit_gui attribute_base=concat( $attribute_base, '_ezorl_edit_object_', $relation.contentobject_id ) html_class='half' attribute=$version_attribute}
                                                    </fieldset>
                                                {else}
                                                    <label>{$version_attribute.contentclass_attribute.name}:</label>
                                                    {attribute_edit_gui attribute_base=concat( $attribute_base, '_ezorl_edit_object_', $relation.contentobject_id ) html_class='half' attribute=$version_attribute}
                                                {/if}
                                            </div>
                                        {/foreach}
                                        {undef $object $version}
                                    </fieldset>
                                </td>

                                {* Order. *}
                                <td>
                                    <input id="ezcoa-{if ne( $attribute_base, 'ContentObjectAttribute' )}{$attribute_base}-{/if}{$attribute.contentclassattribute_id}_{$attribute.contentclass_attribute_identifier}_order" class="ezcc-{$attribute.object.content_class.identifier} ezcca-{$attribute.object.content_class.identifier}_{$attribute.contentclass_attribute_identifier}" size="2" type="text" name="{$attribute_base}_priority[{$attribute.id}][]" value="{$relation.priority}" />
                                </td>
                            {else}
                                {def $object=fetch( content, object, hash( object_id, $relation.contentobject_id, object_version, $relation.contentobject_version ) )}
                                {* Remove. *}
                                ...
                                {* Order. *}
                                <td>
                                    <input size="2" type="text" name="{$attribute_base}_priority[{$attribute.id}][]" value="{$relation.priority}" />
                                </td>
                                {undef $object}
                            {/if}
                        </tr>
        ...
        {else}
            ...
            {if and( $attribute.is_required, $minElements|eq(0) )}
                {set $minElements = 1}
            {/if}
            {if $minElements|gt(0)}
            ...
            {def $relationTextTrad = concat($relationTextAction, " contain ",  $relationTextNb, " ", $relationTextItem)}
            ...
            {undef $minElements $maxElements $relationTextAction $relationTextItem $relationTextNb $relationTextTrad}
        {/if}
    </div>
{/if}
